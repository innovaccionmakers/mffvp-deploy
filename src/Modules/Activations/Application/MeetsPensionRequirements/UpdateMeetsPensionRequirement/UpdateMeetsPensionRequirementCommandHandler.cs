...
{
    public async Task<Result<MeetsPensionRequirementResponse>> Handle(UpdateMeetsPensionRequirementCommand request, CancellationToken cancellationToken)
    {
        await using DbTransaction transaction = await unitOfWork.BeginTransactionAsync(cancellationToken);

        var entity = await meetspensionrequirementRepository.GetAsync(request.MeetsPensionRequirementId, cancellationToken);
        if (entity is null)
        {
        ...
        return new MeetsPensionRequirementResponse(entity.MeetsPensionRequirementId, entity.AffiliateId, entity.StartDate, entity.ExpirationDate, entity.CreationDate, entity.State);
    }
}