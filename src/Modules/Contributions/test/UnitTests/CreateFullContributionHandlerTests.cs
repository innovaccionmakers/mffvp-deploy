...
public sealed class CreateFullContributionHandlerTests
{
    ...
    public CreateFullContributionHandlerTests()
    {
        ...
        _lookups.Setup(l => l.CodeExists(It.IsAny<string>(), It.IsAny<string>())).Returns(true);
        _lookups.Setup(l => l.CodeIsActive(It.IsAny<string>(), It.IsAny<string>())).Returns(true);
        _lookups.Setup(l => l.OriginRequiresCertification(It.IsAny<string>())).Returns(false);
        ...
        _portRepo.Setup(p => p.BelongsToObjective("P01", 1458)).Returns(true);
    ...
    public async Task Handle_Should_ReturnError6000_When_ClientDoesNotExist()
    {
        ...
        _rules.Setup(r => r.EvaluateAsync(
              ...
              .ReturnsAsync((false,
                             Array.Empty<RuleResultTree>(),
                             new[] { new RuleValidationError("6000", "El Cliente no existe") }));
    ...
    public async Task Handle_Should_ReturnError6267_When_AmountIsZero()
    {
        ...
        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("6267");
        result.Error.Description.Should().Contain("greater than 0");
    }
