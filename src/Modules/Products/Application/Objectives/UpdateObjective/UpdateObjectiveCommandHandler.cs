...
{
    public async Task<Result<ObjectiveResponse>> Handle(UpdateObjectiveCommand request,
        CancellationToken cancellationToken)
    {
        await using var transaction = await unitOfWork.BeginTransactionAsync(cancellationToken);

        var entity = await objectiveRepository.GetAsync(request.ObjectiveId, cancellationToken);
        if (entity is null) return Result.Failure<ObjectiveResponse>(ObjectiveErrors.NotFound(request.ObjectiveId));
        ...
        return new ObjectiveResponse(entity.ObjectiveId, entity.ObjectiveTypeId, entity.AffiliateId,
            entity.AlternativeId, entity.Name, entity.Status, entity.CreationDate);
    }
}