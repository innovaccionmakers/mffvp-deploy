...
namespace MFFVP.ArchitectureTests;
    ...
    [Collection("ClosingTest")]
    public class ClosingTest : BaseTest, IModuleTest
    {
     
        public ClosingTest() : base()
        {
            currentModule = nameof(Monolith.Closing);

            foreach (var module in ClosingModules)
            {
    ...
    public void DbContextPersistenceLayerClasses_WhenInheritanceIsAttempted_ThenItShouldNotBePossible()
    {
        var result = Types.InCurrentDomain()
            ...
            .And()
            .ImplementInterface(typeof(Closing.Application.Abstractions.Data.IUnitOfWork))
            .Should()
            .BeSealed()
            .GetResult();
