...
public class ProductTest : BaseTest, IModuleTest
{
  
    public ProductTest() : base()
    {
        currentModule = nameof(Monolith.Products);

        foreach (var module in ProductsModules)
        {
    ...
    public void DbContextPersistenceLayerClasses_WhenInheritanceIsAttempted_ThenItShouldNotBePossible()
    {
        var result = Types.InCurrentDomain()
            ...
            .And()
            .ImplementInterface(typeof(Products.Application.Abstractions.Data.IUnitOfWork))
            .Should()
            .BeSealed()
            .GetResult();
    ...
    //[Fact]
    //public void Presentation_should_not_depend_on_Domain_nor_Infrastructure()
    //{
    //    var result = Types.InCurrentDomain()
    //        .That()
