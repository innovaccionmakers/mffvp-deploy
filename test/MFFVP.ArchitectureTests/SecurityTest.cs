...
public class SecurityTest : BaseTest, IModuleTest
{
   public SecurityTest() : base()
    {
        currentModule = nameof(Monolith.Security);

        foreach (var module in SecurityModules)
        {
    ...
    public void DbContextPersistenceLayerClasses_WhenInheritanceIsAttempted_ThenItShouldNotBePossible()
    {
        var result = Types.InCurrentDomain()
            ...
            .And()
            .ImplementInterface(typeof(Security.Application.Abstractions.Data.IUnitOfWork))
            .Should()
            .BeSealed()
            .GetResult();
